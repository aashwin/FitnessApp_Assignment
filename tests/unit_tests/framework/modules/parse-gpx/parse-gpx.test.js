...
describe('Framework -> Parse GPX', function () {
    describe('Tests for parseGPX()', function () {
        ...
        it('parseGpx should return error if file is an xml file but not gpx', function (done) {
            gpxParser.parseGpx(path.join(__dirname, '../../../../resources/xml_but_not_gpx.xml'), function (err, obj) {
                expect(err).to.not.be.null;
                expect(err).to.be.a('error');
                expect(err.message).to.contain("Could not parse GPX file");
                done();
            });
        });
    });
